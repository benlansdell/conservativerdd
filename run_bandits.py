from lib.bandits import LinUCB, ThresholdBandit, ThresholdConsBandit, ThresholdMaxConsBandit, ThresholdMaxConsGreedyBandit, \
				GreedyBandit, ConsLinUCB, RarelySwitchingLinUCB, ThresholdBaselineBandit, \
				DeterministicThresholdBandit, expected_regret, expected_regret_per_arm
...
def get_args():
	...
	argparser.add_argument(
		...
		metavar='R',
		default='None',
		help='Starting rate for deterministic bandit')
...
def main():
	...
	starting_rate = float(args.startingrate)
	...
	for alg in algs:	
		...
		elif alg == 'deterministicthreshold':
			BanditAlg = DeterministicThresholdBandit
		...
		for m in range(M):
			...
			if alg == 'conslinucb':
				...
				print("Starting rate is", starting_rate)
				bandit = BanditAlg(generator, delta = delta, starting_rate = starting_rate)
			else:
